...
def validate_shapes(
    ...
    counts = [frame_count, len(points), len(colors), len(poses)]
    if len(set(counts)) != 1:
        raise ValueError(f"frame counts differ: images/points/colors/poses = {counts}")
...
def main() -> None:
    ...
    output = args.output or output_dir / "reconstruction_video.mp4"
    ...
    validate_shapes(points, colors, poses, masks, len(paths))

    rng = np.random.default_rng(args.seed)
    sampled_points: list[np.ndarray] = []
    sampled_colors: list[np.ndarray] = []
    for index in range(len(paths)):
        frame_mask = None if masks is None else masks[index].numpy()
        world, rgb = sample_world_points(
            points[index].numpy(), colors[index].numpy(), poses[index], frame_mask,
    ...
    fig.suptitle("ABot-Recon  |  Online 3D Reconstruction", color="white", fontsize=16)
    ...
    with imageio.get_writer(
        ...
        for index, path in enumerate(paths):
            accumulated_p.append(sampled_points[index])
            accumulated_c.append(sampled_colors[index])
            ...
            ax_image.set_title(f"Input RGB  |  Frame {index + 1:03d}/{len(paths):03d}", color="white")
            ax_image.axis("off")
            ...
            ax_cloud.set_title(f"Accumulated 3D point cloud  |  {len(cloud):,} points", color="white")
